...
const CALIDAD_JPG: u8 = 35; // Calidad más baja para mayor compresión
const CALIDAD_WEBP: u8 = 40; // Calidad más baja para mayor compresión
const LIMITE_WEBP_MB: u64 = 3 * 1024 * 1024;
const CRF_VIDEO: &str = "32"; // Más compresión en video
...
fn main() -> Result<()> {
    ...
    for (i, path) in archivos.iter().enumerate() {
        ...
        pb_principal.set_message(format!(
            "Metadatos: {} | JPG: {} | PNG: {} | HEIC: {}/{} | Videos: {}/{}",
            contador_metadatos,
            contador_jpg,
            contador_png,
            convertidos,
    ...
    println!(
        "📏 Tamaño antes   : {:.2} MB",
        tam_before as f64 / 1024.0 / 1024.0
    );
    println!(
        "📉 Tamaño después : {:.2} MB",
        tam_after as f64 / 1024.0 / 1024.0
    );
    println!(
        "💾 Espacio ahorrado: {:.2} MB ({:.2}%)",
        ahorrado as f64 / 1024.0 / 1024.0,
        porcentaje
    );
    ...
    println!(
        "🎬 Videos optimizados: {} ({:.2} MB ahorrados)",
        videos_optimizados,
        videos_ahorro_kb as f64 / 1024.0
    );
...
fn mostrar_resultados_parciales(
    metadatos: u32,
    jpg: u32,
    png: u32,
    heic_total: u32,
    convertidos: u32,
    videos_total: u32,
    videos_optimizados: u32,
    videos_ahorro_kb: u64,
    tam_before: u64,
    ruta: &str,
) {
    ...
    if let Ok(tam_actual) = calcular_tamano_carpeta(ruta) {
        ...
        println!(
            "💾 Espacio ahorrado hasta ahora: {:.2} MB ({:.2}%)",
            ahorrado as f64 / 1024.0 / 1024.0,
            porcentaje
        );
    }
...
fn calcular_tamano_carpeta(ruta: &str) -> Result<u64> {
    ...
    let output_str =
        String::from_utf8(output.stdout).context("Error convirtiendo output a string")?;
...
fn limpiar_metadatos_archivo(path: &Path) -> Result<()> {
    ...
    match child
        ...
        .context("Error esperando exiftool")?
    {
...
fn optimizar_jpg_archivo(path: &Path) -> Result<()> {
    ...
    match child
        .wait_timeout(timeout)
        .context("Error esperando jpegoptim")?
    {
        Some(status) if status.success() => Ok(()),
...
fn optimizar_png_archivo(path: &Path) -> Result<()> {
    ...
    match child
        .wait_timeout(timeout)
        .context("Error esperando optipng")?
    {
        Some(status) if status.success() => Ok(()),
...
fn convertir_heic_archivo(path: &Path) -> Result<()> {
    ...
    let timeout = Duration::from_secs(TIMEOUT_SECS);
    let heif_ok = match child
        .wait_timeout(timeout)
        .context("Error esperando heif-convert")?
    {
    ...
    if heif_ok {
        ...
        let timeout = Duration::from_secs(TIMEOUT_SECS);
        let webp_ok = match child
            .wait_timeout(timeout)
            .context("Error esperando cwebp")?
        {
...
fn optimizar_video_archivo(path: &Path) -> Result<u64> {
    ...
    match child
        .wait_timeout(timeout)
        .context("Error esperando ffmpeg")?
    {
...
fn limpiar_archivos_temporales(ruta: &str) -> Result<()> {
    ...
    if borrados == 0 {
        ...
        println!(
            "✅ Se eliminaron {} archivos temporales residuales.",
            borrados
        );
    }
    Ok(())
